...
type TreeFolderNode = z.infer<typeof treeFolderNodeSchema>;
type TreeQuestionNode = z.infer<typeof treeQuestionNodeSchema>;
type TreeNode = TreeFolderNode | TreeQuestionNode;

const treeQuestionNodeSchema = z.object({
  id: z.string().min(1),
...
const treeFolderNodeSchema: z.ZodType<{
  id: string;
  type: "folder";
  name: string;
  active: boolean;
...
const treeQuestionnaireNodeSchema: z.ZodType<{
  id: string;
  type: "questionnaire";
  name: string;
  active: boolean;
...
function convertTreeQuestionnaire(root: TreeQuestionnaireNode): Questionnaire {
  ...
  const questionsWithRules = questions.map((question) => {
...
function convertOptions(node: TreeQuestionNode) {
  ...
  return node.answers
    ...
    .map((answer) => ({
      value: getAnswerValue(answer.text),
      title: answer.text,
      order: answer.order,
...
function convertRules(
  ...
  return node.answers
    .filter((answer) => answer.active)
    .map((answer) => {
      const action = convertTransitionAction(answer.transition.action);
      const questionId = resolveTransitionQuestionId(
      ...
      return {
        value: node.answerType === "boolean" ? getBooleanRuleValue(answer.text) : getAnswerValue(answer.text),
        action,
...
function convertAnswerType(answerType: TreeQuestionNode["answerType"]): QuestionnaireAnswerType {
  const map: Record<TreeQuestionNode["answerType"], QuestionnaireAnswerType> = {
    boolean: "boolean",
    singleChoice: "select",
...
function getAnswerValue(text: string): string {
  return text.trim();
}
